...
WHERE 
    ...
    $.ajax({
        ...
        success: (response) => {
            ...
            for (let i = 0; i < respuesta.length; i++) {
                if (!(respuesta[i]["ID"] in diccionarioAsesor)) {
                    ...
                    diccionarioAsesor[respuesta[i]["ID"]] = {
                        ...
                        totalDuracion: parseFloat(respuesta[i]["Duracion"]),
            ...
            console.log("Diccionario Asesor: ", diccionarioAsesor);
            // Llenado del div

            $("#tabla").html("");
            html = "<table class='m-auto'><thead><tr><th>Correo</th><th>Nombre</th><th>Sesiones</th><th>Total Horas Talent</th><th>Duración Media Sesión</th><th>% Horas Prof</th><th>% Horas Talent</th></tr></thead>";
            html += "<tbody>";
            ...
            <td>${
                    convertirMinutosHoras(elem.totalDuracion)
                }</td>
            <td>${
                    convertirMinutosHoras(elem.duracionMediaSesion)
                }</td>
            <td>%${
                    (elem.porcentajeProf)
                }</td>
            <td>%${
                    (elem.porcentajeTalent)
                }</td>
        </tr>`;
            });
