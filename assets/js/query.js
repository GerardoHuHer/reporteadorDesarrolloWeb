// Estas variables son para la cinta de resumen
var registrosTotal;
var horasTotales;
var duracionMedia;
...
// Diccionario contenedor de los filtros.
var diccionario = {};

// Diccionario para información de la vista asesor
var diccionarioAsesor = {};
// Número de la vista que tenemos, 1 resultados, 2 categoría, 3 asesor
var num;
var cate = ["AIA", "DCL", "AEC", "ARC", "DIT", "ACD", "SGT", "LMS", "MRA"];
...
function sendRequestRespuestas() {
  ...
  if (arrCategorias.length !== 0) {
    ...
    for (let i = 0; i < arrCategorias.length; i++) {
      if (i === arrCategorias.length - 1) {
        ...
        queryResultados +=
          " categoria.ID = " + borrar(arrCategorias[i]) + " OR";
      }
    }
    queryResultados += ") ";
  }
  ...
  $.ajax({
    ...
    success: (response) => {
      ...
      html =
        "<table class='m-auto table-resultados'><thead><tr><th>ID</th><th>Correo</th><th>Fecha</th><th>Duración</th><th>Categoría</th><th>Asesor</th></tr></thead>";
      html += "<tbody>";
      for (let i = 0; i < respuesta.length; i++) {
        html += `<tr>
                    <td>${respuesta[i]["ID_Asesoria"]}</td>
                                        <td>${respuesta[i]["Correo"]}</td>
                <td>${respuesta[i]["Fecha"]}</td>
                <td>${convertirMinutosHoras(respuesta[i]["Duracion"])}</td>
                <td>${respuesta[i]["Llave"]}</td>
                <td>${respuesta[i]["Nombre_Asesor"]}</td>
                </tr>`;
      }
      ...
      for (let j = 0; j < respuesta.length; j++) {
        // Sesiones
        if (
          diccionarioCategoria[respuesta[j]["Llave"]].Sesiones.indexOf(
            respuesta[j]["ID_Asesoria"]
          ) === -1
        ) {
          diccionarioCategoria[respuesta[j]["Llave"]].Sesiones.push(
            respuesta[j]["ID_Asesoria"]
          );
          // Total horas Profesores
          diccionarioCategoria[respuesta[j]["Llave"]].TotalHorasProfes +=
            parseFloat(respuesta[j]["Duracion"]);
          horasTotales += parseFloat(respuesta[j]["Duracion"]);
          registrosTotal += 1;
        }
        // Profesores
        if (
          diccionarioCategoria[respuesta[j]["Llave"]].Profesores.indexOf(
            respuesta[j]["Correo"]
          ) === -1
        ) {
          diccionarioCategoria[respuesta[j]["Llave"]].Profesores.push(
            respuesta[j]["Correo"]
          );
          profesores += 1;
        }
        // Horas Talent
        diccionarioCategoria[respuesta[j]["Llave"]].TotalHorasTalent +=
          parseFloat(respuesta[j]["Duracion"]);
      ...
      for (let j = 0; j < respuesta.length; j++) {
        ...
        if (diccionarioCategoria[respuesta[j]["Llave"]].Sesiones.length > 0) {
          diccionarioCategoria[respuesta[j]["Llave"]].DuracionMediaProf =
            totalProf /
            diccionarioCategoria[respuesta[j]["Llave"]].Sesiones.length;
          diccionarioCategoria[respuesta[j]["Llave"]].DuracionMediaTalent =
            totalTalent /
            diccionarioCategoria[respuesta[j]["Llave"]].Sesiones.length;
        }
      }
      ...
      resumen = `<div class="m-auto"><div class="row cinta">
                <div class="col resultado-container-1">Sesiones <p class="dato-numerico">${registrosTotal}</p></div>
                 ...
                 <div class="col resultado-container-3">Duración Media Sesión <p class="dato-numerico">${convertirMinutosHoras(
                   duracionMedia
                 )}</p></div>
                 <div class="col resultado-container-4">Total Hrs. Talent <p class="dato-numerico">${convertirMinutosHoras(
                   horasTotalesTalent
                 )}</p></div>
      ...
      (html += "</tbody></table>"), $("#tablas").html(html);
    },
  });
}
...
function sendRequestCategoria() {
  $("#tabla").html("");
  html =
    "<table class='m-auto'><thead><tr><th>Key</th><th>Nombre</th><th>Sesiones</th><th>Profesores</th><th>Total Horas Prof</th><th>Total Horas TALENT</th><th>Duración Media Prof</th><th>Duración Media TALENT</th></tr></thead>";
  html += "<tbody>";
  ...
  Object.values(diccionarioCategoria).forEach((elem) => {
    html += `<tr>
            <td>${elem.ID}</td>
            <td>${elem.Nombre}</td>
            <td>${elem.Sesiones.length}</td>
            <td>${elem.Profesores.length}</td>
            <td>${convertirMinutosHoras(elem.TotalHorasProfes)}</td>
            <td>${convertirMinutosHoras(elem.TotalHorasTalent)}</td>
            <td>${convertirMinutosHoras(elem.DuracionMediaProf)}</td>
            <td>${convertirMinutosHoras(elem.DuracionMediaTalent)}</td>
        </tr>`;
  });
...
WHERE 
  ...
  $.ajax({
    ...
    success: (response) => {
      ...
      if(respuesta.length === 0){
        html = "<h4>No se encontró un resultado</h4>"
        $("#tablas").html(html);
        return;
      }
      for (let i = 0; i < respuesta.length; i++) {
        ...
        if (!diccionarioAsesor[arrTalents[i]]) {
          ...
          diccionarioAsesor[respuesta[i]["ID"]] = {
            // Id
            id: respuesta[i]["ID"],
            // Correo
          ...
          diccionarioAsesor[respuesta[i]["ID"]].sesiones += 1;
          diccionarioAsesor[respuesta[i]["ID"]].totalDuracion +=
      ...
      for (let i = 0; i < arrTalents.length; i++) {
        ...
        console.log("DIccionario de Asesores:", diccionarioAsesor )
      }
    },
  });
}
...
function convertirMinutosHoras(numeroMinutos) {
  ...
  let minutos = `${parseInt(numeroMinutos) % 60}`;
  if (minutos.length === 1) {
    minutos = "0" + minutos;
  }
